## 👋 Hi,

**Senior Linux Systems & Infrastructure Engineer | Automation, Ansible & OpenShift | Learning Full-Stack Development (Vue.js, Django)**

- 🖥️ 20+ years managing Red Hat, Fedora, SUSE, and openSUSE Linux servers 
- 🔐 Experienced infrastructure security, backup systems, and monitoring (Zabbix)  
- 🐍 Experienced in scripting with **Python**  
- 🌐 Currently into **full-stack web development** using **Vue.js**, and **Django**
- 🧠 Always learning — focused on building secure, performant web applications

---

## 🛠️ Tech Stack
<div align="center">
<img src="https://skillicons.dev/icons?i=html,css,js,ts,vue,nodejs,python,django,postgresql,git,ansible,linux,openshift,kubernetes" /><br/><br/>

**Linux & Infrastructure**  
`RHEL` `Fedora` `SUSE` `openSUSE` `VMware` `LVM` `DataDomain` `Zabbix` `Bash` `Python`

**Web Development**  
`HTML` `CSS` `JavaScript` `Typescript` `Vue.js` `NodeJS` `NestJS` `Python` `Django` `Supabase` `Git`

---
</div>

## 🚧 Current Focus

- Building full-stack projects using Vue.js/Django  
- Practicing REST APIs and component-based frontend development  
- Exploring secure deployment strategies combining Linux + Web

---

## 📫 Contact

- Email: **thelinuxlighthouse@thelinuxlighthouse.com**  
- LinkedIn: [linkedin.com/in/ahmed-husam-elsers](https://linkedin.com/in/ahmed-husam-elsers)  
- YouTube: [The Linux Lighthouse](https://www.youtube.com/@TheLinuxLighthouse)

---

> “Old school system admin. New school developer in progress.”  
